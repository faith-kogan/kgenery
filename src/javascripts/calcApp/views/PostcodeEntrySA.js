...
class PostcodeEntrySA extends Component {
  state = {
  ...
  updateInput = event => {
    ...
    const isServiceablePostcode = Utils.isServiceablePostcodeSA(postcode);
    ...
    if (
      ...
      return this.setState({
        errors: {
          ...this.state.errors,
          postcode: "This offer is only available for properties in South Australia.",
        },
      });
    }
    ...
    return (
      <div>
        <div className="c-input">
          ...
          {state.errors.postcode && (
            <small className="c-input__error c-input--centered">{state.errors.postcode} <br />Not in SA? <a href="/p/100-discount-when-you-join-powershop/">Join here</a></small>
          )}
        </div>
      </div>
    );
