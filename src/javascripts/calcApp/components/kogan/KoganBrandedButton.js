...
const BrandedButton = ({
  ...
  let iconElement = iconName
    ? <i className={`kgnIcon ${iconTypes[iconName]}`} />
    : ''

  if (iconName === "kgnArrowRjight") {
    iconElement = (
      <span className="kgnIcon c-btn--icon-right">
        <svg viewBox="0 0 1 1"><use xlinkHref="images/icons.svg#arrow-right"></use></svg>
      </span>
    )
  }
  ...
  return (
    <button
      className={`${className} kogan-button ${ieFixClass} ${isLoadingClass}`}
